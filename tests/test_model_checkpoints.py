...
from solar_forecast.models.shared.checkpoint_store import TrainingCheckpointStore
from solar_forecast.models.shared.checkpoint_store import training_fingerprint
from solar_forecast.models.cnn_bilstm import adaptive_training as adaptive
from solar_forecast.models.cnn_bilstm import training_workflow as workflow
...
def test_cnn_fixed_training_resumes_after_interrupted_epoch(
    ...
    store = _store(tmp_path)
    original = workflow.train_cnn_bilstm_epoch
    calls = 0
    ...
    monkeypatch.setattr(workflow, "train_cnn_bilstm_epoch", interrupted)
    with pytest.raises(RuntimeError, match="simulated interruption"):
        workflow.train_cnn_bilstm(
            _frame(),
            "target",
    ...
    monkeypatch.setattr(workflow, "train_cnn_bilstm_epoch", resumed)
    artifacts = workflow.train_cnn_bilstm(
        _frame(),
        "target",
        feature_columns=["f0", "f1"],
        sequence_config=SequenceConfig(
...
def test_cnn_legacy_workflow_persists_study_and_final_state(tmp_path: Path):
    ...
    first = workflow.train_cnn_bilstm(**arguments)
    second = workflow.train_cnn_bilstm(**arguments)
...
def test_adaptive_training_restores_bandit_and_epoch_progress(
    ...
    monkeypatch.setattr(adaptive, "train_cnn_bilstm_epoch", interrupted)
    monkeypatch.setattr(
        adaptive,
    ...
    with pytest.raises(RuntimeError, match="adaptive interruption"):
        adaptive.run_adaptive_training(
            CnnBiLstmNetworkConfig(n_features=2, cnn_channels=4, lstm_hidden=4, dense_units=4),
            train_loader=[],
            val_loader=[],
            epochs=3,
            checkpoint_store=store,
    ...
    monkeypatch.setattr(adaptive, "train_cnn_bilstm_epoch", resumed)
