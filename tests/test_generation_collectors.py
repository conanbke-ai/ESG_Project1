...
from solar_forecast.collectors.collection_config import load_source_catalog
from solar_forecast.datasets.archive_standardizer import HistoricalGenerationStandardizationService
...
from solar_forecast.infrastructure.csv_storage import write_standardized_csv
from solar_forecast.collectors.download_naming import build_solar_download_filename
from solar_forecast.collectors.download_naming import is_canonical_solar_download_name
from solar_forecast.collectors.generation_normalizers import EWP_POINT_SCHEMA
from solar_forecast.collectors.generation_normalizers import DailyWideGenerationNormalizer
from solar_forecast.collectors.generation_normalizers import EwpTrainingNormalizer
from solar_forecast.collectors.generation_normalizers import GENERATION_COLUMNS
from solar_forecast.collectors.generation_normalizers import KOSPO_ARCHIVE_INTERVAL_SCHEMA
from solar_forecast.collectors.generation_normalizers import IWEST_RENEWABLE_SCHEMA
from solar_forecast.collectors.generation_normalizers import IWEST_WIDE_SCHEMA
from solar_forecast.collectors.generation_normalizers import KOSPO_WIDE_SCHEMA
from solar_forecast.collectors.generation_normalizers import KoenGenerationNormalizer
...
def test_kma_hourly_chunks_never_exceed_one_year():
    chunks = list(KmaAsosBrowserCollector._chunks(date(2023, 6, 1), date(2025, 6, 1)))
    assert chunks[0] == (date(2023, 6, 1), date(2023, 12, 31))
    assert chunks[-1][1] == date(2025, 6, 1)


def test_kma_chunks_support_leap_day():
    chunks = list(KmaAsosBrowserCollector._chunks(date(2024, 2, 29), date(2025, 3, 1)))
