...
from solar_forecast.reporting import ModelAnalyticsService
...
def test_dashboard_frontend_compares_all_metrics_and_supports_national_search():
    ...
    assert "renderMetricQuartet" in script
    ...
    assert "renderDetailDirectionOptions" in script
    ...
    assert '<option value="${escapeHtml(row.region)}"${row.region === state.selectedRegion ? " selected" : ""}>${escapeHtml(displayRegion(row.region))}</option>' in script
    assert 'data-detail-region="${escapeHtml(row.region)}"' in script
    assert 'aria-label="${escapeHtml(displayRegion(row.region))} 상세 보기"' in script
    ...
    assert "<strong>${escapeHtml(displayRegion(name))}</strong>" in script
    ...
    assert "renderDetailTable(rows, searching, aliasMatch)" in script
