...
이번 변경은 구조상 근거가 있는 개선 후보이며 **정확도 향상 자체를 입증한 것은 아닙니다**. 다음 실험은 Train에서만 적합한 수치 입력 표준화, 입력창 길이, 충분한 학습과 early stopping을 순차 평가합니다. 표준화를 추가할 때는 저장 통계와 모든 재예측 경로의 사용을 함께 구현해야 합니다. XGBoost는 별도로 lag·rolling 특징과 트리 설정을 개선합니다. Hybrid는 독립 채택 구간에서 최선 단일 모델보다 개선된 경우에만 선택합니다.
