...
현재 실행 기준은 [실측 모델 비교 계약](OPTIMIZED_MODEL_BENCHMARK.md)입니다. 아래의
TimeXer·RevIN·S4M·그래프·다발전원 multi-task 제안은 후속 연구 후보이며 이번 구현 대상이
아닙니다. 정기 재학습·드리프트 탐지 자동화도 추가하지 않습니다. XGBoost와 CNN-BiLSTM을
각각 개선하고, 독립 채택 구간에서 개선된 경우에만 Hybrid를 선택하는 기존 목적을 유지합니다.

2026-09-17 실제 원본·PPT 재검토에 따른 적용 사항:

- 발전량 원값을 보존하고 음수·무한대가 lag/rolling으로 재유입되지 않게 합니다. 기존 시간
  해상도 부적격 발전소는 학습 제외하되 전체 기간 진단으로 과거 이력값을 바꾸지 않습니다.
  원인을 알 수 없는 용량 초과·양의 flatline·주간 0은 표시만 합니다.
- ASOS는 발전소 병합 전에 관측소·시각별로 QC와 물리 범위를 검사하고, 결측 사유와 당시 유효한
  관측소 메타데이터를 보존합니다. 결측 강수·야간 일사를 관측된 0으로 간주하지 않습니다.
- CNN은 Train 입력 행에서만 중앙값과 평균·표준편차를 적합합니다. 저장 통계와 피처 순서를
  재예측에 재사용하며 결측 마스크는 0/1로 유지합니다. 타깃은 변환하지 않습니다.
- 날짜 고정 분할과 학습 없는 커버리지 감사 도구를 추가합니다. 데이터 추가 시 경계가 움직이지
  않으며, 분할별 발전소·월·계절·Train 이력 없는 발전소를 확인합니다.

이는 데이터 계약과 실행 경로의 보완이며 성능 향상 증명은 아닙니다. 이하 과거 실험 수치와
모집단은 당시 기록으로만 읽어야 합니다. 현재 확보된 저장소 입력의 모집단은 새 감사 결과를
따르며, 접근할 수 없는 Windows 수집 파일의 행 수를 합산하지 않습니다.
...
2. 통계 표준화
   ...
   - CNN은 Train 입력 행에서 중앙값을 적합한 뒤 같은 행의 평균·표준편차로 수치 입력을
     표준화하고, 같은 피처 순서의 이진 missing mask 채널을 붙입니다. Train에서 전부 비어 있는
     피처도 미래 통계를 빌리지 않고 `0 + mask`로 표현합니다.
   - 후속 연구에서는 발전소별 분포 이동을 줄이는 RevIN 후보를 비교할 수 있습니다. RevIN은 입력 window의
...
2026-09-17 보완: 병합본과 공식 원본의 양의 flatline 차이를 보간의 영향으로 단정할 수 없습니다.
동일 자료 대조에서 병합본의 flatline 8,659행에 비해 공식 원단위는 4행이었지만, 공식 값을
소수점 둘째 자리로 반올림하면 8,817행이었습니다. 따라서 이 차이는 주로 반올림과 관련되며,
flatline만으로 센서 오류·보간 오류를 확정하지 않습니다. 원단위 공식 타깃으로 학습 테이블을
구성하고, 관측된 양의 동일값은 검토 표시와 함께 보존합니다.
