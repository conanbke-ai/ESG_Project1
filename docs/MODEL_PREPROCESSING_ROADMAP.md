...
이 문서는 기존 발표자료의 전처리·모델 비교를 현재 코드와 대조하고, 네 발전사의 서로 다른
...
소수력도 `hydro`로 보존하고 태양광 모델의 18개 발전소에서 제외합니다.
...
위반 flag와 `capacity_factor`를 생성합니다. 음수 발전량만 명백한 물리 위반으로 자동 학습
제외하고, 용량초과·주간 0·flatline은 메타데이터 오류·출력제어·정비와 구별할 수 없으므로
검토 대상으로만 표시합니다.
...
- 추가 데이터 완료: 한국농어촌공사 영암 2020~2025 원문 확보, 2022~2025 개체 안전 정규화,
  2020~2021 quarantine, hash lineage와 후보 품질 gate. 기상 매핑·단위 검토 전에는 학습 미편입.
