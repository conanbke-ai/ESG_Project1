...
33,059.516180 MW이다. 세부지역 320개 원본 표기는 2026-07-01 행정구역 reference를 적용해
270개 지역으로 통합한다. 과거 광주·전남과 새 표기를 `전남광주통합특별시`로 합쳐 16개 최신
광역 기준으로 표시하되 원본 fact와 총량은 변경하지 않는다. 메인 지도에는 세부지역 점을 겹쳐
그리지 않고 광역 단계구분도만 기본 표시하며, 시도명·약칭·세부지역명을 전국에서 검색한다.
광역지역과 세부지역 원천 표기가 여전히 충돌하는 집계 행은 자동 재배정하지 않고 확인 필요 상태로
보존한다. 상세 근거는 `NATIONAL_REGION_REFERENCE_AUDIT.md`에 기록한다.
...
두 HTML은 동일 CSS/JavaScript와 생성 JSON을 공유하지만 역할은 섞지 않는다. 메인 화면의
`national_inventory`는 EPSIS 전국 원본의 사용자용 집계이며, `model_analysis`는 정식 동일 Test
표본의 예측 성능과 태양광 데이터 이상 신호만 담는다. 시도 경계는 로컬에서 제공하고 저채도 OSM
