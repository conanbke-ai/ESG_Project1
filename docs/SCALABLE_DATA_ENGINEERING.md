...
Gold 27개 회사×연도 gzip 파티션은 44,745,838 byte입니다. 실제 XGBoost 계약으로 전체
718,531행을 스캔해 태양광·품질 게이트를 통과한 701,011행만 유지했을 때, 26개 피처와
문맥 컬럼을 포함한 pandas DataFrame은 118.33MB였습니다. 모든 수치 피처·타깃은 `float32`이며
...
> 근거를 통과한 72만 Gold 행을 gzip 파티션으로 처리했습니다. 원본·산출물 SHA-256과 atomic
> write로 lineage와 실패 안전성을 확보하고, 불확실한 44개 발전소는 quarantine했습니다. 학습 시에는
