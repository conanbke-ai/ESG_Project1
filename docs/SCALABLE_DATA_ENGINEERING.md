...
편입 원본 28,203,932 byte가 시간별 long gzip 파티션 32,866,577 byte가 됐습니다. gzip인데도
원본보다 큰 이유는 일별 wide 행의 메타데이터가 24개 시간 행에 반복되기 때문이며, 이 측정은
Parquet columnar encoding 전환의 구체적인 근거로 사용합니다.

Gold 27개 회사×연도 gzip 파티션은 44,910,649 byte입니다. 실제 XGBoost 계약으로 전체
718,531행을 스캔해 태양광·품질 게이트를 통과한 671,731행만 유지했을 때, 26개 피처와
문맥 컬럼을 포함한 pandas DataFrame은 114.68MB였습니다. 모든 수치 피처·타깃은 `float32`이며
...
> 근거를 통과한 72만 Gold 행을 gzip 파티션으로 처리하고, 그중 태양광 품질 게이트를 통과한
> 약 67만 행만 학습 적격으로 선택했습니다. 원본·산출물 SHA-256과 atomic write로 lineage와 실패
> 안전성을 확보하고, 불확실한 44개 발전소는 quarantine했습니다. 학습 시에는
