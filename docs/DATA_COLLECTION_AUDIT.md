...
이 ablation은 legacy ASOS fail-closed 전 18개 안정 표본의 기준선이다. 현재 태양광 Gold
22개·701,011행에서는 같은 rolling-origin 계약으로 다시 평가하고 Optuna를 재개해야 하며, 아래
수치를 새 모집단의 고정 최적값으로 사용하지 않는다.
...
전라도 표본은 전남 태양광 8개소·273,955행, 전남 풍력 2개소·17,520행, 전북 태양광
2개소·22,824행으로 총 12개소·314,299행입니다. 태양광만 보면 10개소·296,779행입니다.
...
재구성 후 결과는 `low` 19곳, `review` 5곳, `high` 0곳입니다. 검토 표시는 공개 자료만으로 원인을
확정할 수 없는 peer/시간 패턴을 사람이 확인해야 한다는 뜻이며 자동 고장 판정이 아닙니다.

검토 대상은 동해바이오화력본부 태양광, 장흥풍력, 화순풍력, 여수태양광, 다송리입니다.
특히 여수태양광은 시간 커버리지 89.12%와 용량초과 4.13%,
다송리는 용량초과 50.63%와 기상 결측 51.02%여서 용량·단위·기상 매핑을 우선 재검증해야 합니다.
이 행들은 원인 flag를 보존하며 자동으로 0 대체하거나 센서 고장으로 판정하지 않습니다.
