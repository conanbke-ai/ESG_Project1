...
history 결측 때문에 행을 삭제하던 정책도 제거했습니다. 현재 전국 Gold는 2,525,434행,
46개 발전소(태양광 43, 풍력 2, 소수력 1), 2013-01-01~2025-12-31입니다. 모델 설정은
`energy_source=solar`와 `quality_train_eligible=true`를 chunk 읽기 단계에서 먼저 적용하며
2,401,306행·43개 태양광 발전소를 유지합니다.
...
전체 registry 68개소 중 46개소가 Gold이고 22개소는 격리됐습니다. 격리 사유는 공개 메타데이터로
방어 가능한 ASOS 매핑을 만들 수 없음 18개소, 복수 ASOS 후보의 모호성 4개소입니다.

기상 피처 관측률은 기온 98.82%, 강수 98.82%, 풍속 98.71%, 습도 98.78%, 일조 98.59%,
일사 89.15%, 전운량 81.76%, 중하층운량 82.10%입니다. 2026 ASOS 전체 CSV는 기상자료개방포털
...
재구성 후 결과는 `low` 40곳, `review` 6곳, `high` 0곳입니다. 검토 표시는 공개 자료만으로 원인을
확정할 수 없는 peer/시간 패턴을 사람이 확인해야 한다는 뜻이며 자동 고장 판정이 아닙니다.

검토 대상은 동해바이오화력본부 태양광, 장흥풍력, 화순풍력, 여수태양광, 다송리,
신인천 1_2단계 주차장입니다. 특히 여수태양광은 시간 커버리지 89.12%와 용량초과 4.13%,
