// Optimized historical benchmark; independent from legacy anomaly analysis.
  function renderBenchmarkSection(benchmark = {}) {
    ...
    return `<section class="surface analysis-section benchmark-section" aria-labelledby="benchmark-title">
      <div class="section-head"><div><h2 id="benchmark-title">모델별 탐색 · 비교 결과</h2><p>실측 기상·발전량으로 평가한 과거 Test 결과입니다. 같은 예측 시간의 공통 표본끼리 비교합니다.</p></div><span class="status-tag">실측 자료 평가</span></div>
  ...
  function renderBenchmarkTask(task) {
    ...
    const testBaseMae = Math.min(...task.models.filter((model) => ["xgboost", "cnn_bilstm"].includes(model.id)).map((model) => model.test_metrics.pooled.mae));
    const hybridTestMae = task.models.find((model) => model.id === "hybrid")?.test_metrics.pooled.mae;
    return (task.provenance?.optimization_scope === "bounded_observed_pilot" ? '<p class="status-message">실측 데이터 일부 범위에서 수행한 제한된 예산의 실험입니다. 전체 데이터 최적화 결과로 해석하지 않습니다.</p>' : "") +
      `<div class="benchmark-selection"><strong>${escapeHtml(formatNumber(task.horizon_hours))}시간 후 · ${escapeHtml(task.selected_label)}</strong><span class="status-tag">검증 구간 선택</span><p>${escapeHtml(task.reason)}</p></div>` +
      (task.selected_model === "hybrid" && hybridTestMae >= testBaseMae ? '<p class="status-message">Hybrid는 검증 구간에서 선택됐지만 최종 Test에서는 최선 단일 모델 대비 MAE 개선을 확인하지 못했습니다. Test 결과로 기존 선택 기록을 바꾸지 않습니다.</p>' : "") +
      ...
      <div class="table-responsive"><table class="data-table"><thead><tr><th>모델</th><th class="numeric">선택 MAE</th><th class="numeric">Test MAE</th><th class="numeric">Test RMSE</th><th class="numeric">Test R²</th></tr></thead><tbody>${task.models.map((model) => `<tr${model.selected ? ' class="is-selected"' : ""}><td><strong>${escapeHtml(model.label)}</strong>${model.selected ? '<span class="status-tag">검증 구간 선택</span>' : ""}</td><td class="numeric">${escapeHtml(metricText("mae", model.selection_metrics.pooled.mae, 6))}</td><td class="numeric">${escapeHtml(metricText("mae", model.test_metrics.pooled.mae, 6))}</td><td class="numeric">${escapeHtml(metricText("rmse", model.test_metrics.pooled.rmse, 6))}</td><td class="numeric">${escapeHtml(metricText("r2", model.test_metrics.pooled.r2, 6))}</td></tr>`).join("")}</tbody></table></div>
      ${persistence && persistence.beats_persistence === false ? '<p class="status-message">선택 모델의 최종 Test MAE가 직전 관측 유지 기준선보다 낮지 않습니다. 모델 채택은 이전 선택 구간에서 결정한 결과입니다.</p>' : ""}
