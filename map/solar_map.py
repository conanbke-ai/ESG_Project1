...
# ===== 지도 생성 =====
logging.info("지도에 발전소 마커 표시 중...")
m = folium.Map(location=[36.5, 127.8], zoom_start=7, tiles="CartoDB positron")
bounds = []

# 광역지역별 FeatureGroup 생성
fg_dict = {region: folium.FeatureGroup(name=region, show=True) for region in unique_regions}

for i, r in tqdm(grouped.iterrows(), total=len(grouped), desc="지도 마커 추가"):
    lat, lon = float(r['위도']), float(r['경도'])
    color = pick_region_color(r['대표광역'])
    radius = (r['발전소수'] ** 0.5) * 1.5 + 2
    popup_html = (
        f"<b>{r['세부지역']}</b><br>"
        f"발전소 수: {r['발전소수']}<br>"
        f"총 설비용량: {r['총설비용량']:.2f} MW"
    )
    folium.CircleMarker(
        location=[lat, lon],
        radius=radius,
        ...
        tooltip=r['세부지역']
    ).add_to(fg_dict[r['대표광역']])
    bounds.append([lat, lon])

# 모든 FeatureGroup 지도에 추가
for fg in fg_dict.values():
    fg.add_to(m)

# 범례 추가
legend_items = ''.join(
    f'<div><span style="display:inline-block;width:12px;height:12px;background:{color};border-radius:50%;margin-right:6px;border:1px solid #333;"></span>{name}</div>'
...
# ===== HTML 결합 (좌측 그래프 / 우측 지도) =====
final_html = f"""
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title>발전소 지도 대시보드</title>
<style>
body {{
  display: flex;
  flex-wrap: wrap;
