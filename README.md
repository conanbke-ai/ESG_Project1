...
한국남동발전·한국남부발전·한국동서발전·한국서부발전이 공개한 전국 분포 태양광 발전소를 대상으로 발전량을 예측하고, 예측 잔차와 공개 기상자료를 이용해 이상징후를 분석·알림하는 프로젝트입니다.

원본 표준화 계층은 태양광뿐 아니라 공개 파일에 함께 들어 있는 풍력·수력을 보존합니다. 태양광
모델은 `energy_source=solar`만 명시적으로 선택하며, 다른 발전원을 태양광 출력에 합치지 않습니다.

이 프로젝트에서 `전국`은 위 4개 발전공기업이 공개한 태양광 발전소의 전국 분포를 뜻하며 국내 모든 민간·공공 설비의 전수자료를 의미하지 않습니다. 공개된 정비·고장 이력이 없으므로 설비 고장을 판정하거나 원인으로 확정하지 않습니다.
...
4개 발전사 파일은 각 공식 홈페이지의 공공데이터 게시물 또는 그 게시물이 연결한
...
기존에 보관한 네 발전사 원본 전체를 공통 스키마로 바꾸고 학습 파일까지 만들려면 다음을 실행합니다.

```bash
python app.py prepare-data
```

`prepare-data`는 `file/solar_data_file/`의 86개 원본을 4,146,757개 시간 행의 회사별 gzip CSV 파티션으로 변환하고,
...
매핑만 가져와 `file/standardized/model_ready.csv`를 생성합니다. 기존 병합 타깃과 공식 원본의
...
실제 4개사 파일의 행 수·기간·결측·중복 검증 결과는
[`docs/DATA_COLLECTION_AUDIT.md`](docs/DATA_COLLECTION_AUDIT.md)에 기록했습니다.

### 추가 발전소 후보

공개 범위를 넓히기 위해 한국농어촌공사 영암 2020~2025 과거 파일도 실제로 내려받았습니다.
...
대구·신안, 한국중부발전 169개소 안심구역 자료, 전력거래소 지역 집계의 용도와 우선순위는
...
결측 stress test용으로 `model_ready.csv`에 계속 보존합니다. 이 수치는 정식 Test 성능이 아니라
...
그 밖의 결측과 초기 168시간 history 결측은 그대로 유지합니다. 이 정책으로 종전보다 3,192개
cold-start 행을 더 보존해 모델 테이블은 331,968행이 됐습니다.
...
`file/standardized/model_ready.csv`, `generation_mwh`, `selected_v3_physics_aware_day_ahead`의
...
학습 job은 한 파일에 모인 모든 발전소에 같은 전역 시간 경계를 적용해 네 역할을 내부에서 나눕니다.
