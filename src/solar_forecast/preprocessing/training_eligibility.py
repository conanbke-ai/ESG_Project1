...
def audit_training_eligibility(
    ...
    *,
    ...
    for plant_id, plant in eligible_target.groupby("plant_id", observed=True, sort=True):
        ...
        status = "STRUCTURAL_REJECT" if hard_reasons else "STRUCTURALLY_ELIGIBLE"
        plants.append({
    ...
    candidates = [item for item in plants if item["status"] == "STRUCTURALLY_ELIGIBLE"]
    rejected = [item for item in plants if item["status"] == "STRUCTURAL_REJECT"]
    return {
        "contract": ELIGIBILITY_CONTRACT,
        "selection_stage": "structural_eligibility_before_admission_and_model_readiness",
