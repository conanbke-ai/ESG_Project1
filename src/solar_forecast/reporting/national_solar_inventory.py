...
class AdministrativeRegionReference:
    ...
    def from_json(cls, path: str | Path) -> "AdministrativeRegionReference":
        ...
        location_search_aliases = {
            _normalize_inventory_text(alias): _normalize_inventory_text(target)
            for alias, target in location_aliases_value.items()
            if _normalize_inventory_text(alias)
        }
        for alias, target in location_search_aliases.items():
            target_region = canonical_region(
                _first_location_token(target), aliases=aliases
            )
        ...
        for region, value in centroids_value.items():
            coordinates = _parse_korean_coordinates(value)
...
class ReviewedLocationOverrideSet:
    ...
    def from_json(
        ...
        *,
        ...
        for index, raw in enumerate(raw_entries, start=1):
            ...
            missing_text = [key for key in required_text if not _normalize_inventory_text(raw.get(key))]
            if missing_text:
                raise InventoryConfigurationError(
                    f"reviewed location override entry {index} has empty fields: "
                    + ", ".join(missing_text)
                )
            override_id = _normalize_inventory_text(raw["id"])
            if override_id in override_ids:
                raise InventoryConfigurationError(
                    f"reviewed location override id is duplicated: {override_id}"
                )
            override_ids.add(override_id)
            if _normalize_inventory_text(raw["review_status"]) != "approved":
                raise InventoryConfigurationError(
                    f"reviewed location override is not approved: {override_id}"
                )
            confidence = _normalize_inventory_text(raw["confidence"])
            if confidence not in {"high", "medium"}:
                raise InventoryConfigurationError(
                    f"reviewed location override confidence is invalid: {override_id}"
                )
            if not re.match(r"^https://", _normalize_inventory_text(raw["evidence_url"])):
                raise InventoryConfigurationError(
                    f"reviewed location override evidence_url must use HTTPS: {override_id}"
                )
            try:
                date.fromisoformat(_normalize_inventory_text(raw["evidence_reference_date"]))
            except ValueError as error:
                raise InventoryConfigurationError(
            ...
            expected_records = _parse_inventory_integer(raw.get("expected_records"))
            if expected_records != len(row_numbers):
                raise InventoryConfigurationError(
                    "reviewed location override expected_records does not match rows: "
                    f"{override_id}"
                )
            expected_capacity = _parse_inventory_decimal(raw.get("expected_capacity_mw"))
            ...
            new_region = _normalize_inventory_text(raw["new_region"])
            if (
            ...
            new_subregion = _normalize_inventory_text(raw["new_subregion"])
            ...
            entries.append(
                ReviewedLocationOverride(
                    ...
                    old_region=_normalize_inventory_text(raw["old_region"]),
                    old_subregion=_normalize_inventory_text(raw["old_subregion"]),
                    new_region=new_region,
                    new_subregion=new_subregion,
                    expected_records=expected_records,
                    expected_capacity_mw=expected_capacity,
                    evidence_provider=_normalize_inventory_text(raw["evidence_provider"]),
                    evidence_url=_normalize_inventory_text(raw["evidence_url"]),
                    evidence_reference_date=_normalize_inventory_text(raw["evidence_reference_date"]),
                    match_method=_normalize_inventory_text(raw["match_method"]),
                    confidence=confidence,
                    reason=_normalize_inventory_text(raw["reason"]),
                )
            )
        return cls(
    ...
    def apply(
        ...
        if entry is None:
            return None
        actual_region = _normalize_inventory_text(record.values.get("광역지역", ""))
        actual_subregion = _normalize_inventory_text(record.values.get("세부지역", ""))
    ...
    def validate_complete(self) -> dict[str, Any]:
        ...
        return {
            ...
            "applied_capacity_mw": _decimal_to_float(sum(self._applied_capacity.values(), Decimal("0"))),
        }
...
class NationalInventoryService:
    ...
    def build(self) -> dict[str, Any]:
        ...
        inventory = {
            ...
            "summary": {
                ...
                "total_capacity_mw": _decimal_to_float(total.capacity_mw),
            ...
            "quality": {
                ...
                "exact_duplicate_capacity_mw": _decimal_to_float(duplicate_capacity),
                ...
                "source_region_conflict_capacity_mw": _decimal_to_float(
                    source_region_conflict_capacity
                ),
                "reviewed_location_override_records": (
                    reviewed_location_override_records
                ),
                "reviewed_location_override_capacity_mw": _decimal_to_float(
                    reviewed_location_override_capacity
                ),
                "unresolved_region_conflict_records": (
                    unresolved_region_conflict_records
                ),
                "unresolved_region_conflict_capacity_mw": _decimal_to_float(
    ...
    def _accumulate_record(
        ...
        *,
        ...
        energy_source = _normalize_inventory_text(values.get("발전원", ""))
        ...
        capacity = _parse_inventory_decimal(values.get("설비용량", ""))
        duplicate = duplicates.is_duplicate(record.raw_values)
        region_raw = _normalize_inventory_text(values.get("광역지역", ""))
        source_region = canonical_region(
            region_raw, aliases=self.region_reference.aliases
        )
        source_subregion = _normalize_inventory_text(values.get("세부지역", ""))
        ...
        for column in REQUIRED_COLUMNS:
            if not _normalize_inventory_text(values.get(column, "")):
    ...
    def _is_footer(record: InventoryCsvRecord) -> bool:
        return (
            _normalize_inventory_text(record.values.get("발전기명", "")) == "통합"
            and not _normalize_inventory_text(record.values.get("광역지역", ""))
            and not _normalize_inventory_text(record.values.get("세부지역", ""))
        )
    ...
    def _footer_quality(
        ...
        declared_count = _parse_inventory_integer(footer.values.get("호기", ""))
        declared_capacity = _parse_inventory_decimal(footer.values.get("설비용량", ""))
        difference = (
        ...
        return {
            ...
            "declared_capacity_mw": _decimal_to_float(declared_capacity),
            "record_count_matches": declared_count == total.records,
            "capacity_matches": (
                difference is not None
                and abs(difference) <= FOOTER_CAPACITY_TOLERANCE_MW
            ),
            "capacity_difference_mw": _decimal_to_float(difference),
        }
        ...
        return [
            {
                ...
                "capacity_mw": _decimal_to_float(totals[region].capacity_mw),
            }
    ...
    def _location_records(
        ...
        for key, value in cache.items():
            ...
            if coordinates is None:
                invalid_entries += 1
                continue
            clean_key = _normalize_inventory_text(key)
            ...
            records.append(
                {
                    ...
                    "capacity_mw": _decimal_to_float(aggregate.capacity_mw),
                    "latitude": latitude,
                    "longitude": longitude,
...
def canonical_region(
    ...
    *,
    ...
    cleaned = _normalize_inventory_text(value)
    ...
    *,
    ...
    text = canonical_location(
        ...
        (
            ...
            else canonical_region(_first_location_token(value), aliases=aliases)
        ),
        aliases=aliases,
        canonical_regions=canonical_regions,
    )
...
def source_region_conflict(
    ...
    *,
    ...
    first = _first_location_token(value)
    return (
        first in aliases
...
def _first_location_token(value: object) -> str:
    text = _normalize_inventory_text(value)
...
def _parse_inventory_decimal(value: object) -> Decimal | None:
    text = _normalize_inventory_text(value).replace(",", "")
    if not text:
        return None
    try:
        result = Decimal(text)
...
def _parse_inventory_integer(value: object) -> int | None:
    number = _parse_inventory_decimal(value)
...
def _parse_korean_coordinates(value: Sequence[float] | object) -> tuple[float, float] | None:
