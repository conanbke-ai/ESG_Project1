...
from solar_forecast.datasets.asos_weather_store import exclusive_asos_collection, merge_weather_rows
...
class KmaAsosBrowserCollector:
    ...
    def _merge_by_year(self, downloads: list[Path]) -> list[Path]:
        ...
        for path in downloads:
            header, values = self._read_csv(path)
            for value in values:
                if len(value) != len(header):
                    raise ValueError("Downloaded ASOS CSV has a malformed row")
                rows.append(dict(zip((name.strip() for name in header), value)))
        return merge_weather_rows(self.config.existing_weather_dir, rows)

    def collect(self) -> CollectionResult:
        with exclusive_asos_collection(self.config.existing_weather_dir):
            return self._collect_browser_observations()

    def _collect_browser_observations(self) -> CollectionResult:
