...
class EwpTrainingDataCollector:
    ...
    def collect(self) -> CollectionResult:
        destination = self.config.output_dir / self.name / self.spec.filename
        ...
        normalized = destination.parent / "normalized" / "ewp_solar_training_2020_2024_utf8.csv"
...
class DataGoFileCollector:
    ...
    def collect(self) -> CollectionResult:
        destination = self.config.output_dir / self.spec.source / self.spec.filename
        destination.parent.mkdir(parents=True, exist_ok=True)
        ...
        if self.normalizer is not None:
            normalized = destination.parent / "normalized" / f"{destination.stem}_normalized.csv"
...
class KoenHomepageCollector:
    ...
    def collect(self) -> CollectionResult:
        ...
        downloader = KoenBrowserDownloader(base)
        files: list[Path] = []
        for year in years:
        ...
        for source in files:
            destination = base / "normalized" / f"{source.stem}_normalized.csv"
