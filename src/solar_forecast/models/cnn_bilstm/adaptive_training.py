...
def run_adaptive_training(
    ...
    model = initial_model.to(device) if initial_model is not None else build_cnn_bilstm_network(
        model_cfg,
        device=device,
    )
    ...
    for epoch in range(start_epoch, loop_end):
        ...
        train_loss = train_cnn_bilstm_epoch(model, train_loader, criterion, optimizer, device)
